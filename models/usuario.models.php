...
class Usuarios
{
    ...
    public function Insertar($Nombre, $Direccion, $Telefono, $Correo, $Parroquia, $Canton, $Provincia)
    {
        $con = new ClaseConectar();
        $con = $con->ProcedimientoConectar();
        $cadena = "INSERT into Usuarios(Nombre,Direccion,Correo,Telefono, Correo, Parroquia, Canton, Provincia) values ('$Nombre', '$Direccion', '$Telefono', '$Correo', '$Parroquia', '$Canton', '$Provincia')";

        if (mysqli_query($con, $cadena)) {
            $UsRoles = new Usuarios_Roles();

            return $UsRoles->Insertar(mysqli_insert_id($con), $SucursalId);
        } else {
